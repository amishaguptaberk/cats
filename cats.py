...
def about(subject):
    ...
    # BEGIN PROBLEM 2
    return lambda paragraph: any([x in split(remove_punctuation(lower(paragraph))) for x in subject])
    # END PROBLEM 2
...
def accuracy(typed, source):
    ...
    for i in range(min(len(source_words) , len(typed_words))): 
        if source_words[i] == typed_words[i]:
            count_similar += 1
    return count_similar * 100/ len(typed_words) 
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if min(differences) > limit : 
        return typed_word
    return word_list[differences.index(min(differences))]
...
def feline_fixes(typed, source, limit):
    ...
    if limit == 0:
        if typed == source:
            return 0 
        return 1
    new_limit = int(typed[0] != source[0])
    return new_limit + feline_fixes(typed[1:], source[1:], limit-new_limit)
    # END PROBLEM 6
...
def time_per_word(words, times_per_player):
    ...
    for player in times_per_player:
        time_differences.append([player[i+1] - player[i] for i in range(len(player) - 1)])
    return match(words, time_differences)
    # END PROBLEM 9
...
def fastest_words(match):
    ...
    for i in word_indices:
        player_times = [time(match, j, i) for j in player_indices]
        fastest_players[player_times.index(min(player_times))].append(get_word(match, i))
